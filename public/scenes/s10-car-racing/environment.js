...
Environment.prototype.createSky = function()
{
    ...
    sky.position.z = -Environment.GROUND_LENGTH ;
    sky.scale.x = 5.5;
    sky.scale.y = 13;
...
{    
    ...
    //Remove the right rail, to let the car can leave the road
...
{    
	...
	if (this.textureFinishLine)
	{
        ...
        texture = THREE.ImageUtils.loadTexture('./images/0.png');
	}
	else
	{
		texture = null;
...
Environment.prototype.createSigns = function()
{
   ...
   var texture = null;	
    for (var i=1; i< 54; i++){
        texture = THREE.ImageUtils.loadTexture('./images/set0/'+padDigits(i,2) + ".jpg");

        var sign = new THREE.Mesh( new THREE.PlaneGeometry( Environment.FINISH_SIGN_WIDTH, 
			Environment.FINISH_SIGN_HEIGHT*3 ), 
			new THREE.MeshBasicMaterial( 
			{ color:  0xFFFFFF, 
					shininess:100, ambient: 0x333333, map:texture } 
			)
    ...
    var offset = 1600;
	sign.position.z =offset +  100*(1 - i) + Car.CAR_LENGTH * 2;
	sign.position.y = Environment.FINISH_SIGN_Y + Environment.GROUND_Y+ 1;
	console.log('z of sign', sign.position.z);
...
Environment.GROUND_LENGTH = 8000;
Environment.ROAD_WIDTH = 8;
Environment.ROAD_LENGTH = 4000; //times 1000,  
